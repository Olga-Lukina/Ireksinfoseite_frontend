          ...
          <figure>
            <router-link :to="productlink(product)">
              <img
                ...
                :src="require(`/public/images_slider/${product.image}`)"
                :alt="product.name"
              />
            </router-link>
          </figure>
...
export default {
  ...
  methods: {
    productlink(product) {
      const parentSlug = product.parentslug ? product.parentslug + '/' : '';
      return `/products/${parentSlug}${product.slug}`;
    },
  },
};
</script>
